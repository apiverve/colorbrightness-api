...
class ColorbrightnessValidationException implements Exception {
  final List<String> errors;

  ColorbrightnessValidationException(this.errors);

  @override
  String toString() => 'ColorbrightnessValidationException: ${errors.join("; ")}';
}
...
/// Color Brightness Calculator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/colorbrightness?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [hex] (required) - Hex color value (with or without # prefix) [format: hexColor]
class ColorbrightnessClient {
  final String apiKey;
  ...
  ColorbrightnessClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/colorbrightness',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ColorbrightnessValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ColorbrightnessValidationException(errors);
    }
  }

  /// Execute a request to the Color Brightness Calculator API.
  ///
  /// Parameters are validated before sending the request.
  Future<ColorbrightnessResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ColorbrightnessException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ColorbrightnessResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ColorbrightnessException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ColorbrightnessException('Resource not found');
      } else {
        throw ColorbrightnessException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ColorbrightnessException) rethrow;
      throw ColorbrightnessException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Color Brightness Calculator API client.
class ColorbrightnessException implements Exception {
  final String message;

  ColorbrightnessException(this.message);

  @override
  String toString() => 'ColorbrightnessException: $message';
}
